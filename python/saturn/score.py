"""
Code from Nathan's scoring script.
"""
import os
import sys
import re
...
class ClassificationResult(object):
    ...
    def __init__(self, labels, predicted_labels, predicted_prbs,
        ...
        # print self.auPRC, self.auPRCscikitlearn_calc_auPRC(positives,
        # predicted_prbs)
...
def verify_file_and_build_scores_array(
    ...
    while True:
        ...
        assert len(
            t_match) == 1, "Line %i in the labels file did not match the expected pattern '(\S+\t\d+\t\d+)\t(.+?)\n'" % t_line_num
        ...
        try:
            ...
            raise InputError(
                "The score at line %i in the submitted file can not be interpreted as a float" % s_line_num)
        scores.append(score)
    ...
    if t_line_num < s_scored_line_num:
        raise InputError(
            "The submitted file has more rows than the reference file")
    if t_line_num > s_scored_line_num:
        raise InputError(
            "The reference file has more rows than the reference file")
