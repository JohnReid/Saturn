...
repeats %>%
    filter('Simple_repeat' == repeatclass, '(C)n' == repeattype) %>%
    summarise(n = n())
repeats %>%
...
```{r featsCount}
feats.count <- data.frame(
    feat.name = names(feature.dfs),
    n.repeats = sapply(feature.dfs, nrow),
    n.nz.ranges = sapply(features, function(f) sum(0 != runValue(f)))) %>%
    mutate(ratio = n.nz.ranges / n.repeats) %>%
    arrange(ratio)
feats.count
```
