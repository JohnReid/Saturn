...
#
devtools::load_all()
rmarkdown::render('expr-features.Rmd')
```
...
nearest.feat <-
    with(mcols(nearest),
        DataFrame(
            similarity = Rle(cut(similarity, seq(-epsilon, 1+epsilon, length.out = n.bins + 1))),
            distance = Rle(cut(log10(distance+1), seq(-epsilon, log10(max(distance+1))+epsilon, length.out = n.bins + 1)))))
...
```{r saveDist}
saveRDS(nearest.feat, nearest.gene.feature.file())
```
...
for (.cell in cells$cell) {
    ...
    saveRDS(DataFrame(nearest.expr = Rle(cell.expr$TPM.avg)), nearest.expr.feature.file(.cell))
}
```
